  ...
  const renderTimeSheetsTable = (tHead, tBody) => {
    return (
      <table className="table-fixed text-left">
        ...
        <tbody>
          {(t(tBody, {}, { returnObjects: true }) as TimeSheetElement[]).map(
            (elem, index) => (
              <tr key={`td-${index}`}>
                <td className="w-24 font">{elem.time}</td>
                <td>{elem.title}</td>
              </tr>
            )
          )}
      ...
      {type === "timesheets" && (
        <>
          <Image
            ...
            objectFit="cover"
          />
          <section className="relative py-2 sm:py-30">
            <CenterBox layout="text">
              <h1 className="font-serif text-5xl pb-4">{t("footer.title")}</h1>
              <div className="m-auto items-center h-auto sm:h-auto justify-between flex">
              ...
              <div className="d-flex justify-content-start">
                <p>
                  <a
                    href="https://www.horneoresany.sk/klastor/"
                    target="_blank"
                  >
                    {t("footer.link")}
                  </a>
